...
def load_file(filepath):
    dataframe = pd.read_csv(filepath, header=None, usecols = [1], nrows = 2000)
    return dataframe.to_numpy()

def load_y(filepath):
    dataframe = pd.read_csv(filepath, header=None, usecols = [1])
    return dataframe.to_numpy()
...
def load_dataset_group(group, prefix=''):
    ...
    filenames = list()
    # rest
    filenames += glob.glob(filepath+'/rest*_ppg.csv')
    # step
    filenames += glob.glob(filepath+'/step*_ppg.csv')
    # squat
    filenames += glob.glob(filepath+'/squat*_ppg.csv')
    # load input data
    X = load_group(filenames, filepath)
    # load class output
    y = load_y(prefix +group+ '/y_' + group + '.csv') #change the path and check the size of y
    return X, y
...
trainX, trainy, testX, testy = load_dataset('/Users/TOOR/Desktop/PPG_ACC_dataset_with_CSV_files_temp/')
...
# takes the train and test dataset, fits a model on the training dataset, 
# evaluates it on the test dataset, and returns an estimate of the models performance
...
# then 1/20 s will correspond to 20 consecutive values 
# 105 tri-axial accelerometer signals
